...
import { Search, Filter, RotateCcw, Wallet, Users, Trash2 } from 'lucide-react';

const SettlementsToolbar = ({ search, setSearch, paymentMethod, setPaymentMethod, status, setStatus, groupId, setGroupId, groups, onClear, onDeleteSelected, hasSelected }) => {
  return (
    <div className="flex flex-col gap-4 mb-6 relative z-20">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-3 rounded-2xl border shadow-sm transition-all duration-300 bg-white dark:bg-[#16181d] border-slate-200 dark:border-slate-800/60">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto no-scrollbar pb-1 md:pb-0">
          ...
          {hasSelected && (
            <button onClick={onDeleteSelected} className="p-2 rounded-xl border border-red-200 dark:border-red-900/50 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors shrink-0" title="Delete Selected">
              <Trash2 size={16} />
            </button>
          )}
