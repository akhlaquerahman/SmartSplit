  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900 dark:bg-black text-white p-3 rounded-xl shadow-xl border border-slate-700/50 text-xs">
        ...
        <p className="font-black text-sm">₹{payload[0].value.toLocaleString()}</p>
      </div>
    );
  }
  return null;
...
const ExpensesAnalytics = ({ stats }) => {
  const categoryData = stats?.categoryBreakdown?.length > 0 ? stats.categoryBreakdown : [{ name: 'No Data', value: 1 }];
        ...
        <div className="h-[200px] w-full flex items-center justify-center relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-lg font-black text-slate-900 dark:text-white">
              ₹{totalCategoryAmount > 1 ? totalCategoryAmount.toLocaleString(undefined, { maximumFractionDigits: 0 }) : 0}
            </span>
          </div>
        </div>
      ...
      >
        <div className="flex justify-between items-start">
          <ChartHeader 
            ...
            subtitle="Daily expense processing volume" 
            icon={TrendingUp} 
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-black text-slate-900 dark:text-white">
              ₹{trendData.reduce((acc, curr) => acc + curr.amount, 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}
            </p>
          </div>
        </div>
